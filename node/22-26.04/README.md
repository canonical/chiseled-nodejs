...
This directory contains the image recipes of Chiselled Node.js 22 LTS. These images are smaller in size,
...
$ skopeo --insecure-policy copy \
      oci-archive:node_22_amd64.rock \
      docker-daemon:ubuntu/chiselled-node:22
```
...
```sh
$ docker run -it ubuntu/chiselled-node:22 exec node
...
```json
{
    ...
    "license": "MIT",
    "scripts": {
        "build": "esbuild --target=node22 --bundle --minify --platform=node main.ts --outfile=dist/index.js",
...
# chiselled container
FROM ubuntu/chiselled-node:22 AS final
